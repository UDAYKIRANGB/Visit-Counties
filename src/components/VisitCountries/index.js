...
class VisitCountries extends Component {
  ...
  render() {
    ...
    const vistedCountries = countryList.filter(each => each.isVisited)
    return (
      <BgContainer>
          ...
          {countryList.map(each => (
            <React.Fragment key={each.id}>
              <ListItem>
                <Name>{each.name}</Name>
                {each.isVisited ? (
                  <VistiedText>Visited</VistiedText>
                ) : (
                  <ButtonElement onClick={() => this.onClickVisit(each.id)}>
                    Visit
                  </ButtonElement>
                )}
              </ListItem>
        ...
        {vistedCountries.length === 0 ? (
          <Heading as="p">No Countries Visited Yet</Heading>
        ) : (
          <VistiedCountriesList>
            {vistedCountries.map(each => (
              <CountryCard
                details={each}
                key={each.id}
